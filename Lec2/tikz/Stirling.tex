...
\colorlet{plotlightblue}{blue!10}
...
\def\stirlingengine#1{
  ...
  % FLYWHEEL
  \draw[very thick,plotdarkblue,top color=blue!20!black!40,bottom color=blue!20!black!50,shading
  angle=0]
    (O) circle (\R);
  ...
  % COOLER
  \foreach \x in {-1,1}{
    \draw[plotdarkblue,line width=2,fill=blue!80,xscale=\x]
      (\W/2,-\R-\d-.9*\H) --++ (\ct,0) |-++
      (\cl,\ct) |-++ (-\cl,\ct) |-++
      (\cl,\ct) |-++ (-\cl,\ct) |-++
      (\cl,\ct) |-++ (-\cl,\ct) |-++
      ...
      (-\ct,\ct);
    \draw[plotdarkblue,line width=1.5,fill=red!80,yscale=\x]
      (\R+\d+.9*\H,\W/2) to[out=90,in=0]++ (-2*\ct,2*\ct) --++
      (-.3*\H,0) coordinate (H\x) to[out=180,in=90]++ (-2*\ct,-2*\ct);
  }

  % WALL
  \draw[line width=1.7,plotdarkblue,double=blue!20!black!60,double distance=\T,line cap=rect]
    (-\W/2,-\R-\d) |-++ (\W-\wt,-\H)
